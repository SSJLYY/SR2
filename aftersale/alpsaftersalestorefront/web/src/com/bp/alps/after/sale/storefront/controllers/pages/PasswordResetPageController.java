...
import com.bp.after.sale.facades.data.ForgottenPwdForm;
import com.bp.after.sale.facades.data.UpdatePwdForm;
...
public class PasswordResetPageController extends AbstractPageController
{
	...
	@Resource(name = "alpsAfterSalesCustomerFacade")
	private AlpsAfterSalesCustomerFacade alpsAfterSalesCustomerFacade;
	...
	public String getPasswordRequest(final Model model) throws CMSItemNotFoundException
	{
		...
		storeCmsPageInModel(model, getContentPageForLabelOrId(RESET_PASSWORD_FORM));
			...
			throws CMSItemNotFoundException
	{
		...
		else
		{
			try
			{
				String token = alpsAfterSalesCustomerFacade.forgottenPassword(form.getUsername());
				model.addAttribute("token",urlEncode(token));
			}
			catch (final UnknownIdentifierException unknownIdentifierException)
			{
				LOG.warn("user name: " + form.getUsername() + " does not exist in the database.");
			}
			...
			throws CMSItemNotFoundException
	{
		...
		else
		{
			try
			{
				customerFacade.forgottenPassword(form.getUsername());
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		if (bindingResult.hasErrors())
		{
			prepareErrorMessage(model, RESET_PASSWORD_FORM);
			return ControllerConstants.Views.Pages.Password.PasswordResetChangePage;
		}
		...
		//{
			try
			{
				if(StringUtils.isNotBlank(form.getOldPwd())){
					customerFacade.changePassword(form.getOldPwd(), form.getPwd());
				}else
				{
			...
			catch (final RuntimeException e)
			{
				...
				if(StringUtils.isNotBlank(form.getOldPwd())) return REDIRECT_MY_ACCOUNT;
			}
		//}
