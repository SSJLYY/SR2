...
public class EnvInitFilter extends HybrisOauth2UserFilter
{
			...
			throws IOException, ServletException
	{
		...
		BaseStoreModel storeModel = null;
		if (auth != null && auth instanceof OAuth2Authentication && !((OAuth2Authentication)auth).isClientOnly()) {
			...
			if(userModel instanceof CustomerModel){
				...
				storeModel = customerModel.getStore();
			}
		}

		if(storeModel==null)
		{
			BaseSiteModel baseSiteModel = baseSiteService.getBaseSiteForUID(baseSiteCode);
			if (baseSiteModel != null)
			{
				List<BaseStoreModel> baseStoreList = baseSiteModel.getStores();
				if (CollectionUtils.isNotEmpty(baseStoreList))
				{
					storeModel = baseStoreList.get(0);
				}
			}
		}
